...
const CARD_TONE = {
  ...
  sky: 'bg-sky-wash text-ocean-deep',
}
...
export default function Stories() {
  ...
  return (
    <section className="wash-coral relative overflow-hidden pb-20 pt-20 md:pb-28 md:pt-28">
      <div className="mx-auto max-w-4xl px-5 text-center md:px-8">
        <Eyebrow className="justify-center text-sunset-deep">Nº 06 — Success Stories</Eyebrow>
        <FadeUp delay={0.1}>
          <h2 className="mt-5 font-display display-tight text-4xl font-medium sm:text-5xl md:text-6xl">
            Small habits, <span className="italic grad-text-sunset">new lives.</span>
          </h2>
        </FadeUp>
        ...
        <div className="mt-8 flex items-center justify-center gap-4">
          <Magnetic>
            <button
              ...
              className="grid h-12 w-12 place-items-center rounded-full border border-ink/12 bg-ivory transition-colors hover:bg-sunset-mid hover:text-ivory"
            >
              <HiArrowLeft />
            </button>
          </Magnetic>
            ...
            {STORIES.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === index ? 'w-7 bg-sunset-mid' : 'w-1.5 bg-ink/15'
                }`}
              />
            ))}
          </div>
