...
import { HiArrowDown, HiArrowRight, HiSparkles } from 'react-icons/hi'
...
export default function Hero() {
  ...
  const layerA = { x: useTransform(sx, (v) => v * 16), y: useTransform(sy, (v) => v * 16) }
  const layerB = { x: useTransform(sx, (v) => v * -24), y: useTransform(sy, (v) => v * -24) }
  ...
  return (
    <section
      ...
      ref={ref}
      onMouseMove={onMouse}
      className="wash-sand relative min-h-[100svh] overflow-hidden"
    >
      {/* rising soap bubbles */}
      {BUBBLES.map((b, i) => (
        <span
          key={i}
          className="pointer-events-none absolute bottom-0 animate-bubble rounded-full border border-sky-tone/45 bg-gradient-to-br from-white/65 to-sky-wash/45"
          style={{ left: b.left, width: b.size, height: b.size, animationDelay: `${b.delay}s` }}
        />
      ))}
      <div className="pointer-events-none absolute -left-28 top-28 h-80 w-80 rounded-full bg-sand-soft blur-3xl" />
      <div className="pointer-events-none absolute -right-24 bottom-16 h-96 w-96 rounded-full bg-sky-wash blur-3xl" />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-28 md:px-8 md:pt-36 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-8 lg:pb-24">
        ...
        <motion.div style={{ y: textY }} className="relative z-10">
          <motion.p
            ...
            className="mb-6 inline-flex items-center gap-2 rounded-full border border-champagne/35 bg-ivory/70 px-4 py-2 text-[11px] font-semibold uppercase tracking-widest2 text-champagne-deep backdrop-blur md:text-xs"
          >
            <HiSparkles className="text-champagne" />
            {HERO.eyebrow}
          </motion.p>

          <h1 className="font-display display-tight text-[15vw] sm:text-6xl md:text-7xl xl:text-[6.4rem]">
          ...
          >
            ...
            <Magnetic>
              <a
                href="#categories"
                className="inline-flex items-center gap-2 rounded-full border border-ink/12 bg-ivory/65 px-7 py-4 text-sm font-medium backdrop-blur transition-colors hover:border-champagne hover:text-champagne-deep"
              >
                {HERO.ctaSecondary}
              </a>
            </Magnetic>
          ...
          >
            {HERO.cards.map((c) => (
              <div key={c.label} className="glass rounded-2xl px-5 py-3.5">
                ...
                <p className="mt-0.5 font-display text-xl">{c.value}</p>
              </div>
            ))}
          </motion.div>
        </motion.div>
        ...
        <div className="relative z-0 mx-auto w-full max-w-md lg:max-w-none">
          <motion.div
            style={{ y: imgY }}
            initial={{ clipPath: 'inset(0 100% 0 0 round 2.5rem)' }}
            animate={{ clipPath: 'inset(0 0% 0 0 round 2.5rem)' }}
            transition={{ delay: 2.4, duration: 1.2, ease: [0.76, 0, 0.24, 1] }}
            className="grain relative aspect-[3/4] overflow-hidden rounded-[2.5rem] shadow-2xl shadow-ink/15"
          >
            <img
              src={IMAGES.heroHome}
              alt="A bright, beautifully kept living room"
              className="h-full w-full object-cover"
              fetchpriority="high"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-ink/35 via-transparent to-transparent" />
            <div className="glass-dark absolute bottom-5 left-5 right-5 rounded-2xl p-4 text-ivory">
              <p className="text-[10px] uppercase tracking-widest2 opacity-85">Right now</p>
              <p className="mt-1 font-display text-lg italic">
                64 homes being quietly perfected.
              </p>
            </div>
          </motion.div>

          {/* floating depth imagery */}
          <motion.div
            style={layerA}
          ...
          >
            <img
              ...
              loading="lazy"
              className="h-24 w-32 rounded-3xl object-cover shadow-xl shadow-ink/15 md:h-28 md:w-36"
            />
          </motion.div>
          <motion.div
            style={layerB}
          ...
          <motion.div
            ...
            className="glass absolute -top-6 right-2 flex items-center gap-3 rounded-2xl p-3 pr-5 lg:right-8"
          >
            <span className="grid h-9 w-9 place-items-center rounded-full bg-emerald-soft text-emerald-deep">
              <HiSparkles />
            </span>
            <div>
              <p className="text-xs font-semibold">Sunita is on her way</p>
              <p className="text-[10px] text-stone">Deep clean · arrives 9:00</p>
            </div>
          </motion.div>
        </div>
      </div>
